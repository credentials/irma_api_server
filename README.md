 ...
 * See the section on Testing for how to generate the keys for the testing scripts.
...
Furthermore, you should run the `utils/preparetestjs.sh` script to prepare testing keys, and make sure that you enable these test keys in your `config.json` in `src/main/resources`, for example, using `src/main/resources/config.sample-demo.json` should do the trick.

After this you can run it using:

    node utils/testsp.js http://<SERVER>:8081

where `<SERVER>` refers to the IP address or hostname of your running `irma-api-server` server. Make sure you use an address that the IRMA app can also reach (we usually use a local ip address for testing).
